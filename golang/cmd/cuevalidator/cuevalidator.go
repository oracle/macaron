...
// Return code:
//
//	0 - If the target is successfully extracted or the provenance validation is successful.
...
func main() {
	...
	if *targetPolicy != "" {
		...
		if result == "" {
			handleError("Error: Unable to extract target from policy.", 1)
		}
	...
	if *validatePolicy != "" && *validateProvenance != "" {
		...
		switch result {
		case 1:
			fmt.Println("Validation successful: Provenance conforms with policy.")
			os.Exit(0)
		case 0:
			fmt.Println("Validation failed: Provenance does not conform with policy.")
			os.Exit(2)
		default:
			handleError("Error: Validation encountered an issue.", 1)
		}
		return
	}
