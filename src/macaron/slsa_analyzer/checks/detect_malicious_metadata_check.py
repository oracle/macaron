...
from macaron.malware_analyzer.pypi_heuristics.pypi_source_extractor import PyPISuspiciousCodeExtractor
...
class DetectMaliciousMetadataCheck(BaseCheck):
    ...
    def validate_malware(self, pypi_package_json: PyPIPackageJsonAsset) -> tuple[bool, dict[str, JsonType] | None]:
        ...
        """
        extractor: PyPISuspiciousCodeExtractor = PyPISuspiciousCodeExtractor(pypi_package_json)
        data: dict[str, JsonType] = extractor.get_extracted_data()
        logger.debug(data)
        if data:
            return True, data
        logger.debug("Skipped PyPI source code analysis")
        return False, None
