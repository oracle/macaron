...
class Flit(BaseBuildTool):
    """This class contains the information of the flit build tool."""

    def __init__(self) -> None:
        """Initialize instance."""
        super().__init__(name="flit", language=BuildLanguage.PYTHON, purl_type="pypi")

    def load_defaults(self) -> None:
        """Load the default values from defaults.ini."""
        super().load_defaults()
        if "builder.flit" in defaults:
            for item in defaults["builder.flit"]:
                if hasattr(self, item):
                    setattr(self, item, defaults.get_list("builder.flit", item))

        if "builder.flit.ci.deploy" in defaults:
            for item in defaults["builder.flit.ci.deploy"]:
                if item in self.ci_deploy_kws:
                    self.ci_deploy_kws[item] = defaults.get_list("builder.flit.ci.deploy", item)
