...
class SUPPORTED_ECOSYSTEMS(Enum):
...
def get_macaron_build_tool_name(build_tool_facts: Sequence[BuildToolFacts], target_language: str) -> MacaronBuildToolName | None:
    """
    ...
    for fact in build_tool_facts:
        if str.lower(fact.language) == target_language:
            try:
...
def gen_generic_build_spec(
    ...
    """Return the content of a Buildspec File.
    ...
    -------
    str | None
        The build spec content or None if there is an error.
    ...
    """
    if purl.type not in [e.name for e in SUPPORTED_ECOSYSTEMS]:
        logger.error("Unfortunately, PURL type %s is not currently supported. Supported ecosystems: %s.", purl.type, str([build.name for build in SUPPORTED_ECOSYSTEMS]))
        return None
    ...
    target_language=SUPPORTED_Languages[purl.type].value
    group = purl.namespace
    artifact = purl.name
    version = purl.version
    if version is None:
        logger.error("Missing version for purl %s.", purl.to_string())
        return None
    ...
    SUPPORTED_ECOSYSTEMS[purl.type].value(base_build_spec_dict).resolve_fields(purl)

    return json.dumps(base_build_spec_dict)
